...
> ✅ **Part 2 RESOLVED (additive, regenerated 2026-07-09):** all **79** missing
> tables are modeled in [`services/legacy_parity/models.py`](services/legacy_parity/models.py).
>
> ⚠️ *A prior pass left that file an empty stub (docstring + imports only) even
> though this note claimed it was done.* It has now been **regenerated by
> reflecting the live database schema** — each class mirrors the real columns and
> types 1:1 (more accurate than translating the Sequelize `.init()` defs). The
> module is imported in `main.py`, so all models register on `Base.metadata`
> (156 tables total: 76 service + 79 legacy-parity + 1 core `AuditLogs`). `create_all`
> is a safe no-op for the already-existing tables. These tables have **no
> routers/endpoints** — they exist for schema parity and data migration; wire up
> APIs per feature as needed.
>
> Excluded from `legacy_parity`: `AuditLogs` (already modeled in `core/audit.py`),
> `Allergie`/`ClearingHouse` (already migrated as `Allergies`/`ClearingHouses`),
> and `PanelOld` (dead table).
